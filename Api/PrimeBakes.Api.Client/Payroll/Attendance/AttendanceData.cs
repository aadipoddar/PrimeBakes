...
public static class AttendanceData
{
	...
	public static async Task DeleteTransaction(AttendanceModel attendance, int userId, string formFactor, string platform, decimal? latitude, decimal? longitude) =>
		await ApiClient.Post(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(DeleteTransaction)), attendance, new { userId, formFactor, platform, latitude, longitude });
	...
	public static async Task RecoverTransaction(AttendanceModel attendance, int userId, string formFactor, string platform, decimal? latitude, decimal? longitude) =>
		await ApiClient.Post(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(RecoverTransaction)), attendance, new { userId, formFactor, platform, latitude, longitude });
	...
	public static async Task<int> SaveTransaction(AttendanceModel attendance, int userId, string formFactor, string platform, decimal? latitude, decimal? longitude) =>
		await ApiClient.Post<int>(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(SaveTransaction)), attendance, new { userId, formFactor, platform, latitude, longitude });
}
