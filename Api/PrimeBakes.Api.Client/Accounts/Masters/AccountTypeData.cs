...
public static class AccountTypeData
{
	...
	public static async Task DeleteTransaction(AccountTypeModel accountType, int userId, string platform) =>
		await ApiClient.Post(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(DeleteTransaction)), accountType, new { userId, platform });

	public static async Task RecoverTransaction(AccountTypeModel accountType, int userId, string platform) =>
		await ApiClient.Post(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(RecoverTransaction)), accountType, new { userId, platform });

	public static async Task<int> SaveTransaction(AccountTypeModel accountType, int userId, string platform) =>
		await ApiClient.Post<int>(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(SaveTransaction)), accountType, new { userId, platform });
}
