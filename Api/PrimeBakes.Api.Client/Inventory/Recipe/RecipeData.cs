...
public static class RecipeData
{
	...
	public static async Task DeleteTransaction(RecipeModel recipe, int userId, string formFactor, string platform, decimal? latitude, decimal? longitude) =>
		await ApiClient.Post(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(DeleteTransaction)), recipe, new { userId, formFactor, platform, latitude, longitude });

	public static async Task<int> SaveTransaction(RecipeModel recipe, List<RecipeDetailModel> recipeDetails, int userId, string formFactor, string platform, decimal? latitude, decimal? longitude) =>
		await ApiClient.Post<int>(Helper.MakeRouteFromEndpointFunction(_endpoint, nameof(SaveTransaction)),
			new RecipeSaveRequest(recipe, recipeDetails), new { userId, formFactor, platform, latitude, longitude });
}
