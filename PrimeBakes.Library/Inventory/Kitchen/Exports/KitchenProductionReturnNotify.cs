...
internal static class KitchenProductionReturnNotify
{
	...
	private static async Task KitchenProductionReturnMail(int kitchenProductionReturnId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = await KitchenProductionReturnInvoiceExport.ExportInvoice(kitchenProductionReturnId, InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = await KitchenProductionReturnInvoiceExport.ExportInvoice(kitchenProductionReturnId, InvoiceExportType.PDF);
