...
public partial class SaleItemReport : IAsyncDisposable
{
	...
	private async Task ExportExcel()
	{
		...
		try
		{
			...
			var (stream, fileName) = await SaleItemReportExcelExport.ExportReport(
	...
	private async Task ExportPdf()
	{
		...
		try
		{
			...
			var (stream, fileName) = await SaleItemReportPDFExport.ExportReport(
					_transactionOverviews,
					dateRangeStart,
					dateRangeEnd,
					_showAllColumns,
	...
	private async Task DownloadPdfInvoice(int transactionId, string transactionNo)
	{
		...
		try
		{
			...
			{
				var stockTransfer = _transactionTransferOverviews.FirstOrDefault(st => st.TransactionNo == transactionNo);
				var (pdfStream, fileName) = await StockTransferInvoicePDFExport.ExportInvoice(stockTransfer.MasterId);
				await SaveAndViewService.SaveAndView(fileName, pdfStream);
			}
			else if (transactionId < 0)
			{
				var (pdfStream, fileName) = await SaleReturnInvoicePDFExport.ExportInvoice(Math.Abs(transactionId));
				await SaveAndViewService.SaveAndView(fileName, pdfStream);
			}
			else
			{
	...
	private async Task DownloadExcelInvoice(int transactionId, string transactionNo)
	{
		...
		try
		{
			...
			{
				var stockTransfer = _transactionTransferOverviews.FirstOrDefault(st => st.TransactionNo == transactionNo);
				var (excelStream, fileName) = await StockTransferInvoiceExcelExport.ExportInvoice(stockTransfer.MasterId);
				await SaveAndViewService.SaveAndView(fileName, excelStream);
			}
			else if (transactionId < 0)
			{
				var (excelStream, fileName) = await SaleReturnInvoiceExcelExport.ExportInvoice(Math.Abs(transactionId));
				await SaveAndViewService.SaveAndView(fileName, excelStream);
			}
			else
			{
