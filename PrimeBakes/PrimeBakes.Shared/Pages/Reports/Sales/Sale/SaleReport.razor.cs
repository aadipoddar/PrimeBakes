...
public partial class SaleReport : IAsyncDisposable
{
	...
	private async Task ExportExcel()
	{
		...
		try
		{
			...
			var (stream, fileName) = await SaleReportExcelExport.ExportReport(
	...
	private async Task ExportPdf()
	{
		...
		try
		{
			...
			var (stream, fileName) = await SaleReportPdfExport.ExportReport(
				   _transactionOverviews,
					dateRangeStart,
					dateRangeEnd,
					_showAllColumns,
	...
	private async Task DownloadPdfInvoice(int transactionId, string transactionNo)
	{
		...
		try
		{
			...
			{
				var stockTransfer = _transactionTransferOverviews.FirstOrDefault(st => st.TransactionNo == transactionNo);
				var (pdfStream, fileName) = await StockTransferInvoicePDFExport.ExportInvoice(stockTransfer.Id);
	...
	private async Task DownloadExcelInvoice(int transactionId, string transactionNo)
	{
		...
		try
		{
			...
			{
				var stockTransfer = _transactionTransferOverviews.FirstOrDefault(st => st.TransactionNo == transactionNo);
				var (excelStream, fileName) = await StockTransferInvoiceExcelExport.ExportInvoice(stockTransfer.Id);
