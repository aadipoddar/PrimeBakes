...
public partial class KitchenIssuePage
{
    ...
    private async Task SaveTransaction(bool savePDF = false, bool saveExcel = false)
    {
        ...
        try
        {
            ...
            if (savePDF)
            {
                var (pdfStream, pdfFileName) = await KitchenIssueInvoiceExport.ExportInvoice(_kitchenIssue.Id, InvoiceExportType.PDF);
                await SaveAndViewService.SaveAndView(pdfFileName, pdfStream);
            }

            if (saveExcel)
            {
                var (excelStream, excelFileName) = await KitchenIssueInvoiceExport.ExportInvoice(_kitchenIssue.Id, InvoiceExportType.Excel);
    ...
    private async Task DownloadPdfInvoice()
    {
        ...
        try
        {
            ...
            var (pdfStream, fileName) = await KitchenIssueInvoiceExport.ExportInvoice(Id.Value, InvoiceExportType.PDF);
    ...
    private async Task DownloadExcelInvoice()
    {
        ...
        try
        {
            ...
            var (excelStream, fileName) = await KitchenIssueInvoiceExport.ExportInvoice(Id.Value, InvoiceExportType.Excel);
