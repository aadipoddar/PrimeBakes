...
public partial class PurchasePage
{
    ...
    private async Task SaveTransaction(bool savePDF = false, bool saveExcel = false)
    {
        ...
        try
        {
            ...
            if (savePDF)
            {
                var (pdfStream, pdfFileName) = await PurchaseInvoiceExport.ExportInvoice(_purchase.Id, InvoiceExportType.PDF);
                await SaveAndViewService.SaveAndView(pdfFileName, pdfStream);
            }

            if (saveExcel)
            {
                var (excelStream, excelFileName) = await PurchaseInvoiceExport.ExportInvoice(_purchase.Id, InvoiceExportType.Excel);
