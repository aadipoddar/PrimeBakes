...
public static class GenerateCodes
{
	...
	public static async Task<string> GenerateBillTransactionNo(BillModel bill, SqlDataAccessTransaction sqlDataAccessTransaction = null)
	{
		...
		if (lastBill is not null)
		{
			...
			if (lastTransactionNo.StartsWith($"{locationPrefix}{financialYear.YearNo}{billPrefix}"))
			{
				var lastNumberPart = lastTransactionNo[(locationPrefix.Length + financialYear.YearNo.ToString().Length + billPrefix.Length)..];
				if (int.TryParse(lastNumberPart, out int lastNumber))
				{
					int nextNumber = lastNumber + 1;
					return await CheckDuplicateCode($"{locationPrefix}{financialYear.YearNo}{billPrefix}{nextNumber:D6}", 6, CodeType.Bill, sqlDataAccessTransaction);
				}
			}
		}
		return await CheckDuplicateCode($"{locationPrefix}{financialYear.YearNo}{billPrefix}000001", 6, CodeType.Bill, sqlDataAccessTransaction);
	}
