...
{
	public static byte[] GenerateThermalBill(SaleThermalBundle bundle)
	{
		using var bitmap = RenderReceipt(bundle);
		return ThermalPrintUtil.BitmapToEscPosBytes(bitmap);
	}

	public static byte[] GenerateThermalBillPng(SaleThermalBundle bundle)
	{
		using var bitmap = RenderReceipt(bundle);
		return ThermalPrintUtil.BitmapToPngBytes(bitmap);
	}

	private static SKBitmap RenderReceipt(SaleThermalBundle bundle)
	{
		var (sale, saleDetails, products, company, party, customer, currentDateTime) = bundle;
		...
		y = DrawBillDetails(canvas, sale, party, customer, width, y);
		y = DrawItemDetails(canvas, sale, saleDetails, products, width, y);
		y = DrawTotalDetails(canvas, sale, saleDetails, width, y);
		y = DrawPaymentModes(canvas, sale, width, y);
		y = DrawFooter(canvas, sale, currentDateTime, width, y);
	...
	private static float DrawBillDetails(SKCanvas canvas, SaleOverviewModel sale, LedgerModel party, CustomerModel customer, int width, float y)
	{
	...
	private static float DrawItemDetails(SKCanvas canvas, SaleOverviewModel sale, List<SaleDetailModel> saleDetails, List<ProductModel> products, int width, float y)
	{
	...
	/// </summary>
	private static float DrawTotalDetails(SKCanvas canvas, SaleOverviewModel sale, List<SaleDetailModel> saleDetails, int width, float y)
	{
	...
	private static float DrawFooter(SKCanvas canvas, SaleOverviewModel sale, DateTime currentDateTime, int width, float y)
	{
