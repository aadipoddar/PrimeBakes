...
internal static class PDFReportExportUtil
{
	...
	internal static MemoryStream ExportToPdf<T>(
		IEnumerable<T> data,
		string reportTitle,
		...
		Dictionary<string, string> customSummaryFields = null)
	{
		...
		try
		{
			using (PdfDocument pdfDocument = new())
			{
				...
				AddBrandingFooter(pdfDocument, currentDateTime);
	...
	/// </summary>
	private static void AddBrandingFooter(PdfDocument document, DateTime currentDateTime)
	{
		try
		{
