...
internal static class ExcelInvoiceExportUtil
{
	...
	internal static MemoryStream ExportInvoiceToExcel<T>(
		...
		Dictionary<string, string> summaryFields = null) where T : class
	{
		...
		try
		{
			...
			currentRow = DrawInvoiceHeader(worksheet, currentRow);
			...
			// 7. Branding Footer
			currentRow = DrawBrandingFooter(worksheet, currentRow, invoiceData.CurrentDateTime);
	...
	/// </summary>
	private static int DrawInvoiceHeader(IWorksheet worksheet, int startRow)
	{
		...
		try
		{
			const string logoResourceName = "PrimeBakes.Exports.Utils.Resources.logo_full.png";
	...
	/// </summary>
	private static int DrawBrandingFooter(IWorksheet worksheet, int startRow, DateTime currentDateTime)
	{
