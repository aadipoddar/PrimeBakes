...
internal static class ExcelReportExportUtil
{
	...
	internal static MemoryStream ExportToExcel<T>(
		IEnumerable<T> data,
		string reportTitle,
		...
		Dictionary<string, string> customSummaryFields = null)
	{
		...
		// Walked several times below; materialise once so lazy projections aren't re-run per pass.
		data = data as IList<T> ?? [.. data];
		...
		try
		{
			using (ExcelEngine excelEngine = new())
			{
				...
				ApplyFinalFormatting(worksheet, effectiveColumnOrder.Count, currentDateTime);
		...
		Dictionary<string, string> headerMetadata = null)
	{
		...
		companyRange.Merge();
		companyRange.Text = CommonSecrets.DatabaseName;
	...
	/// </summary>
	private static void ApplyFinalFormatting(IWorksheet worksheet, int columnCount, DateTime currentDateTime)
	{
		try
		{
