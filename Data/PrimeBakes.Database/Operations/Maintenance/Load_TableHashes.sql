...
BEGIN
	...
	SET @SQL = N'SELECT CONVERT(NVARCHAR(MAX), ' + QUOTENAME(@KeyColumn) + N') AS [KeyValue], HASHBYTES(''SHA2_256'', CONCAT(' + @Columns + N')) AS [Hash] FROM ' + QUOTENAME(@TableName) + N' ORDER BY ' + QUOTENAME(@KeyColumn) + N';';
