CREATE PROCEDURE [dbo].[Delete_TableData_By_Keys]
...
BEGIN
	...
	SET @SQL = N'DELETE FROM ' + QUOTENAME(@TableName) + N' WHERE ' + QUOTENAME(@KeyColumn) +
		N' IN (SELECT CONVERT(' + @KeyType + N', [value]) FROM STRING_SPLIT(@Keys, CHAR(31)));';
