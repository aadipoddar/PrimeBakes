...
internal static class KitchenIssueReturnNotify
{
	...
	private static async Task KitchenIssueReturnMail(int kitchenIssueReturnId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = await KitchenIssueReturnInvoiceExport.ExportInvoice(kitchenIssueReturnId, InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = await KitchenIssueReturnInvoiceExport.ExportInvoice(kitchenIssueReturnId, InvoiceExportType.PDF);
