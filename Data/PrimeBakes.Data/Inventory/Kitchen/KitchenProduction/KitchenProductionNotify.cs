...
internal static class KitchenProductionNotify
{
	...
	private static async Task KitchenProductionMail(int kitchenProductionId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = KitchenProductionInvoiceExport.ExportInvoice(await KitchenProductionData.LoadInvoiceBundle(kitchenProductionId), InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = KitchenProductionInvoiceExport.ExportInvoice(await KitchenProductionData.LoadInvoiceBundle(kitchenProductionId), InvoiceExportType.PDF);
