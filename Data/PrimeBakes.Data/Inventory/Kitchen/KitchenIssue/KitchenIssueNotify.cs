...
internal static class KitchenIssueNotify
{
	...
	private static async Task KitchenIssueMail(int kitchenIssueId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		var emailData = new TransactionMailing.TransactionEmailData
		{
			...
			Remarks = kitchenIssue.Remarks,
			Differences = type == NotifyType.Updated ? (await AuditTrailData.LoadLastAuditTrailByTableRecord(InventoryNames.KitchenIssue, kitchenIssue.TransactionNo)).RecordValue : null
		};

		// For update emails, include before and after invoices
		if (type == NotifyType.Updated && previousInvoice.HasValue)
		{
			var (afterStream, afterFileName) = KitchenIssueInvoiceExport.ExportInvoice(await KitchenIssueData.LoadInvoiceBundle(kitchenIssueId), InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = KitchenIssueInvoiceExport.ExportInvoice(await KitchenIssueData.LoadInvoiceBundle(kitchenIssueId), InvoiceExportType.PDF);
