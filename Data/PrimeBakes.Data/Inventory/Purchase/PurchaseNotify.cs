...
internal static class PurchaseNotify
{
	...
	private static async Task PurchaseMail(int purchaseId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = PurchaseInvoiceExport.ExportInvoice(await PurchaseData.LoadInvoiceBundle(purchaseId), InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = PurchaseInvoiceExport.ExportInvoice(await PurchaseData.LoadInvoiceBundle(purchaseId), InvoiceExportType.PDF);
