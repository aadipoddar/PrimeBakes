...
internal static class PurchaseReturnNotify
{
	...
	private static async Task PurchaseReturnMail(int purchaseReturnId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = PurchaseReturnInvoiceExport.ExportInvoice(await PurchaseReturnData.LoadInvoiceBundle(purchaseReturnId), InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = PurchaseReturnInvoiceExport.ExportInvoice(await PurchaseReturnData.LoadInvoiceBundle(purchaseReturnId), InvoiceExportType.PDF);
