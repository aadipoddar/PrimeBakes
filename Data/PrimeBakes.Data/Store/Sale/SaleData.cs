...
public static class SaleData
{
	...
	public static async Task<SaleInvoiceBundle> LoadInvoiceBundle(int transactionId)
	{
		...
		return new(transaction, transactionDetails, company, location, party, order, await CommonData.LoadCurrentDateTime());
	}
	...
	public static async Task<SaleThermalBundle> LoadThermalBundle(int saleId)
	{
		...
		if (sale.PartyId.HasValue && sale.PartyId.Value > 0)
			party = await CommonData.LoadTableDataById<LedgerModel>(AccountNames.Ledger, sale.PartyId.Value);

		CustomerModel customer = null;
		if (sale.CustomerId.HasValue && sale.CustomerId.Value > 0)
			customer = await CommonData.LoadTableDataById<CustomerModel>(StoreNames.Customer, sale.CustomerId.Value);

		return new(sale, saleDetails, products, company, party, customer, await CommonData.LoadCurrentDateTime());
	}
