...
internal static class SaleReturnNotify
{
	...
	private static async Task SaleReturnMail(int saleReturnId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = SaleReturnInvoiceExport.ExportInvoice(await SaleReturnData.LoadInvoiceBundle(saleReturnId), InvoiceExportType.PDF);
		...
		{
			var (pdfStream, pdfFileName) = SaleReturnInvoiceExport.ExportInvoice(await SaleReturnData.LoadInvoiceBundle(saleReturnId), InvoiceExportType.PDF);
