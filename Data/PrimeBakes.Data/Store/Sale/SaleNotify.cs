...
internal static class SaleNotify
{
	...
	private static async Task SaleMail(int saleId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = SaleInvoiceExport.ExportInvoice(await SaleData.LoadInvoiceBundle(saleId), InvoiceExportType.PDF);
		...
		{
			var (pdfStream, pdfFileName) = SaleInvoiceExport.ExportInvoice(await SaleData.LoadInvoiceBundle(saleId), InvoiceExportType.PDF);
