...
internal static class OrderNotify
{
	...
	private static async Task OrderMail(int orderId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = OrderInvoiceExport.ExportInvoice(await OrderData.LoadInvoiceBundle(orderId), InvoiceExportType.PDF);
		...
		else
		{
			...
			var (pdfStream, pdfFileName) = OrderInvoiceExport.ExportInvoice(await OrderData.LoadInvoiceBundle(orderId), InvoiceExportType.PDF);
