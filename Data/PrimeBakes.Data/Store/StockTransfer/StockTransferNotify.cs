...
internal static class StockTransferNotify
{
	...
	private static async Task StockTransferMail(int stockTransferId, NotifyType type, (MemoryStream, string)? previousInvoice = null)
	{
		...
		{
			var (afterStream, afterFileName) = StockTransferInvoiceExport.ExportInvoice(await StockTransferData.LoadInvoiceBundle(stockTransferId), InvoiceExportType.PDF);
		...
		{
			var (pdfStream, pdfFileName) = StockTransferInvoiceExport.ExportInvoice(await StockTransferData.LoadInvoiceBundle(stockTransferId), InvoiceExportType.PDF);
