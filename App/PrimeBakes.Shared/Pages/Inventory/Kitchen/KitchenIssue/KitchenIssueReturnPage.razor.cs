...
namespace PrimeBakes.Shared.Pages.Inventory.Kitchen.KitchenIssue;

public partial class KitchenIssueReturnPage
{
	...
	private KitchenIssueReturnItemCartModel _selectedCart = new();
	private KitchenIssueReturnModel _kitchenIssueReturn = new();

	private List<RawMaterialStockSummaryModel> _stockSummary = [];
	private List<CompanyModel> _companies = [];
	private List<KitchenModel> _kitchens = [];
	private List<RawMaterialModel> _rawMaterials = [];
	private List<KitchenIssueReturnItemCartModel> _cart = [];
	...
	private SfGrid<KitchenIssueReturnItemCartModel> _sfCartGrid;
	...
	private async Task<bool> LoadExistingTransaction()
	{
		...
		_kitchenIssueReturn = await CommonData.LoadTableDataById<KitchenIssueReturnModel>(InventoryNames.KitchenIssueReturn, Id.Value);
		if (_kitchenIssueReturn is null || _kitchenIssueReturn.Id == 0)
		{
	...
	{
		if (!await DataStorageService.LocalExists(StorageFileNames.KitchenIssueReturnDataFileName))
			return false;

		try
		{
			_kitchenIssueReturn = JsonSerializer.Deserialize<KitchenIssueReturnModel>(await DataStorageService.LocalGetAsync(StorageFileNames.KitchenIssueReturnDataFileName));
			return _kitchenIssueReturn is not null;
		}
		catch
		{
	...
	{
		if (_kitchenIssueReturn.CompanyId > 0)
			_selectedCompany = _companies.FirstOrDefault(s => s.Id == _kitchenIssueReturn.CompanyId) ?? _companies.FirstOrDefault();
		else
			_selectedCompany = _companies.FirstOrDefault();

		if (_kitchenIssueReturn.KitchenId > 0)
			_selectedKitchen = _kitchens.FirstOrDefault(s => s.Id == _kitchenIssueReturn.KitchenId) ?? _kitchens.FirstOrDefault();
		else
			_selectedKitchen = _kitchens.FirstOrDefault();

		_kitchenIssueReturn.CompanyId = _selectedCompany.Id;
		_kitchenIssueReturn.KitchenId = _selectedKitchen.Id;

		_selectedFinancialYear = await CommonData.LoadTableDataById<FinancialYearModel>(AccountNames.FinancialYear, _kitchenIssueReturn.FinancialYearId);
	}

	private async Task LoadItems()
	{
		_rawMaterials = await PurchaseData.LoadRawMaterialByPartyPurchaseDateTime(0, _kitchenIssueReturn.TransactionDateTime, useLocalDB: true);
		_rawMaterials = [.. _rawMaterials.OrderBy(s => s.Name)];

		_stockSummary = await RawMaterialStockData.LoadRawMaterialStockSummaryByDate(_kitchenIssueReturn.TransactionDateTime, _kitchenIssueReturn.TransactionDateTime);
	}
	...
	private async Task ResolveCart()
	{
		try
		{
			...
			if (await DataStorageService.LocalExists(StorageFileNames.KitchenIssueReturnCartDataFileName))
				_cart = JsonSerializer.Deserialize<List<KitchenIssueReturnItemCartModel>>(await DataStorageService.LocalGetAsync(StorageFileNames.KitchenIssueReturnCartDataFileName));
		}
		catch (Exception ex)
		{
	...
	{
		if (_kitchenIssueReturn.Id <= 0)
			return false;

		var existingCart = await CommonData.LoadTableDataByMasterId<KitchenIssueReturnDetailModel>(InventoryNames.KitchenIssueReturnDetail, _kitchenIssueReturn.Id);
	...
	{
		_kitchenIssueReturn.TransactionDateTime = value;
		await LoadItems();
	}
	#endregion
	...
	private async Task EditCartItem(KitchenIssueReturnItemCartModel cartItem)
	{
	...
	private async Task RemoveItemFromCart(KitchenIssueReturnItemCartModel cartItem)
	{
	...
	private void UpdateFinancialDetails()
	{
		...
		_kitchenIssueReturn.CompanyId = _selectedCompany.Id;
		_kitchenIssueReturn.KitchenId = _selectedKitchen.Id;
		_kitchenIssueReturn.TotalItems = _cart.Count;
		_kitchenIssueReturn.TotalQuantity = _cart.Sum(x => x.Quantity);
		_kitchenIssueReturn.TotalAmount = _cart.Sum(x => x.Total);
	}

	private async Task PrepareSave()
	{
		_selectedFinancialYear = await FinancialYearData.LoadFinancialYearByDateTime(_kitchenIssueReturn.TransactionDateTime);
		if (_selectedFinancialYear is not null && !_selectedFinancialYear.Locked)
			_kitchenIssueReturn.FinancialYearId = _selectedFinancialYear.Id;
		else
			await _toastNotification.ShowAsync("Invalid Transaction Date", "The selected transaction date does not fall within an active financial year.", ToastType.Error);

		if (Id is null)
			_kitchenIssueReturn.TransactionNo = await GenerateCodes.GenerateKitchenIssueReturnTransactionNo(_kitchenIssueReturn);

		var currentDateTime = await CommonData.LoadCurrentDateTime();
		_kitchenIssueReturn.Status = true;
		_kitchenIssueReturn.TransactionDateTime = DateOnly.FromDateTime(_kitchenIssueReturn.TransactionDateTime).ToDateTime(new TimeOnly(currentDateTime.Hour, currentDateTime.Minute, currentDateTime.Second));
		_kitchenIssueReturn.CreatedAt = currentDateTime;
		_kitchenIssueReturn.LastModifiedAt = currentDateTime;
		var platform = await PlatformInfo.GetPlatformInfo();
		_kitchenIssueReturn.CreatedFormFactor = platform.FormFactor;
		_kitchenIssueReturn.CreatedPlatform = platform.Platform;
		_kitchenIssueReturn.CreatedLatitude = platform.Latitude;
		_kitchenIssueReturn.CreatedLongitude = platform.Longitude;
		_kitchenIssueReturn.LastModifiedFormFactor = platform.FormFactor;
		_kitchenIssueReturn.LastModifiedPlatform = platform.Platform;
		_kitchenIssueReturn.LastModifiedLatitude = platform.Latitude;
		_kitchenIssueReturn.LastModifiedLongitude = platform.Longitude;
		_kitchenIssueReturn.CreatedBy = _user.Id;
		_kitchenIssueReturn.LastModifiedBy = _user.Id;
	}
	...
	private async Task SaveTransactionFile(bool prepareSave = false)
	{
		...
		try
		{
			...
			if (_cart.Count == 0 || _kitchenIssueReturn.Id > 0)
			{
				await DeleteLocalFiles();
				return;
			}

			await DataStorageService.LocalSaveAsync(StorageFileNames.KitchenIssueReturnDataFileName, JsonSerializer.Serialize(_kitchenIssueReturn));
			await DataStorageService.LocalSaveAsync(StorageFileNames.KitchenIssueReturnCartDataFileName, JsonSerializer.Serialize(_cart));
		}
		catch (Exception ex)
		{
	...
	private async Task SaveTransaction(bool savePDF = false, bool saveExcel = false)
	{
		...
		try
		{
			...
			_kitchenIssueReturn.Id = await KitchenIssueReturnData.SaveTransaction(_kitchenIssueReturn, items);
			_kitchenIssueReturn = await CommonData.LoadTableDataById<KitchenIssueReturnModel>(InventoryNames.KitchenIssueReturn, _kitchenIssueReturn.Id);
	...
	{
		if (_kitchenIssueReturn.Id <= 0 || (_isProcessing && !force))
			return;
		...
		try
		{
			...
			var decodeTransactionNo = await DecodeCode.DecodeTransactionNo(_kitchenIssueReturn.TransactionNo, !isExcel, isExcel, CodeType.KitchenIssueReturn);
	...
	#region Utilities
	private async Task OnCartGridContextMenuItemClicked(ContextMenuClickEventArgs<KitchenIssueReturnItemCartModel> args)
	{
		switch (args.Item.Id)
		{
	...
	{
		await DataStorageService.LocalRemove(StorageFileNames.KitchenIssueReturnDataFileName);
		await DataStorageService.LocalRemove(StorageFileNames.KitchenIssueReturnCartDataFileName);
	}
