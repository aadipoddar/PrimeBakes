...
public partial class KitchenIssueItemMonthlyReport : IAsyncDisposable
{
	...
	private List<KitchenIssueItemMonthlySummaryModel> _monthlySummaries = [];

	private SfGrid<KitchenIssueItemMonthlySummaryModel> _sfGrid;
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			await AuditTrailData.SaveAuditTrail(new()
			{
				...
				TableName = InventoryRouteNames.KitchenIssueItemMonthlyReport,
	...
	private void BuildMonthlySummaries()
	{
		...
		_monthHeaders = KitchenIssueItemMonthlySummaryModel.BuildMonthHeaders(financialYearStart);
		...
		_monthlySummaries = [.. _transactionOverviews
			...
			.Select(g =>
			{
				var issues = g.Where(t => t.Quantity > 0).ToList();

				var summary = new KitchenIssueItemMonthlySummaryModel
				{
					ItemId = g.Key,
					...
					MinimumRate = issues.Count == 0 ? 0 : issues.Min(t => t.Rate),
					MaximumRate = issues.Count == 0 ? 0 : issues.Max(t => t.Rate),
					LastRate = issues.Count == 0 ? 0 : issues.OrderBy(t => t.TransactionDateTime).Last().Rate,
					TransactionCount = g.Select(t => t.MasterId).Distinct().Count(),
					KitchenCount = g.Select(t => t.KitchenId).Distinct().Count(),
					FirstIssueDateTime = g.Min(t => t.TransactionDateTime),
					LastIssueDateTime = g.Max(t => t.TransactionDateTime)
				};
		...
		for (var index = 0; index < _monthlySummaries.Count; index++)
		{
			...
			summary.MonthsSinceLastIssue = summary.LastIssueDateTime is null
				? 0
				: ((_currentDateTime.Year - summary.LastIssueDateTime.Value.Year) * 12) + _currentDateTime.Month - summary.LastIssueDateTime.Value.Month;
		}
	}

	private static int FindMonthIndex(KitchenIssueItemMonthlySummaryModel summary, decimal value)
	{
	...
	private async Task ExportReport(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = KitchenIssueItemMonthlyReportExport.ExportReport(
				_monthlySummaries,
				_monthHeaders,
				_currentDateTime,
