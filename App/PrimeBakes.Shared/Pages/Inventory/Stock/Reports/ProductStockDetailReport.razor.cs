...
public partial class ProductStockDetailReport : IAsyncDisposable
{
	...
	private async Task DeleteTransaction(int id, string transactionNo)
	{
		...
		try
		{
			...
			var platform = await PlatformInfo.GetPlatformInfo();
			await ProductStockData.DeleteProductStockAdjustment(id, _user.Id, platform.FormFactor, platform.Platform, platform.Latitude, platform.Longitude);
	...
	private async Task ExportReport(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = ProductStockReportExport.ExportDetailsReport(
				_stockDetails,
