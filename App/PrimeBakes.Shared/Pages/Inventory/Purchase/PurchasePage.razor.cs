...
public partial class PurchasePage
{
	...
	{
		_rawMaterials = await PurchaseData.LoadRawMaterialByPartyPurchaseDateTime(_purchase.PartyId, _purchase.TransactionDateTime, useLocalDB: true);
	...
	{
		_purchase.OtherChargesPercent = _purchase.TotalAfterTax > 0 ? value / _purchase.TotalAfterTax * 100 : 0;
		await SaveTransactionFile();
	}
	...
	private async Task OnCashDiscountAmountChanged(decimal value)
	{
		...
		_purchase.CashDiscountPercent = totalAfterOtherCharges > 0 ? value / totalAfterOtherCharges * 100 : 0;
		await SaveTransactionFile();
	}
