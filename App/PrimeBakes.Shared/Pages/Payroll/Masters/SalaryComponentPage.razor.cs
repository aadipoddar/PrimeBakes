...
public partial class SalaryComponentPage
{
	...
	private SalaryComponentModel _salaryComponent = new() { Sequence = 1, Rounding = true, ShowOnPayslip = true };

	private List<SalaryComponentModel> _salaryComponents = [];
	private readonly List<string> _componentTypes = [.. Enum.GetNames<SalaryComponentTypes>()];
	...
	{
		_salaryComponents = await CommonData.LoadTableData<SalaryComponentModel>(PayrollNames.SalaryComponent);
		_salaryComponents = [.. _salaryComponents.OrderBy(c => c.Sequence)];

		if (!_showDeleted)
			_salaryComponents = [.. _salaryComponents.Where(c => c.Status)];
	...
	private async Task SaveTransaction()
	{
		...
		try
		{
			...
			await SalaryComponentData.SaveTransaction(_salaryComponent, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
	...
	private async Task EditSelectedItem()
	{
		...
		_salaryComponent = await CommonData.LoadTableDataById<SalaryComponentModel>(PayrollNames.SalaryComponent, selectedRecords[0].Id);
		if (_salaryComponent is null)
		{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			if (isRecover) await SalaryComponentData.RecoverTransaction(salaryComponent, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await SalaryComponentData.DeleteTransaction(salaryComponent, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = SalaryComponentExport.ExportMaster(_salaryComponents, await CommonData.LoadCurrentDateTime(), isExcel ? ReportExportType.Excel : ReportExportType.PDF);
