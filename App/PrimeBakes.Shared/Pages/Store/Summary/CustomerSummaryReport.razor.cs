...
public partial class CustomerSummaryReport : IAsyncDisposable
{
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			var allSales = CommonData.LoadTableDataByDate<SaleOverviewModel>(StoreNames.SaleOverview, fromDate, toDate, useLocalDB: true);
			var allReturns = CommonData.LoadTableDataByDate<SaleReturnOverviewModel>(StoreNames.SaleReturnOverview, fromDate, toDate, useLocalDB: true);
			var allBills = CommonData.LoadTableDataByDate<BillOverviewModel>(RestaurantNames.BillOverview, fromDate, toDate, useLocalDB: true);
	...
	private async Task ExportReport(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = CustomerSummaryReportExport.ExportReport(
				_customerSummaries,
