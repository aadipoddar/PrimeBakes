...
public partial class KOTCategoryPage
{
	...
	private async Task LoadData()
	{
		...
		if (!_showDeleted)
			_kotCategories = [.. _kotCategories.Where(kc => kc.Status)];
	...
	private async Task SaveTransaction()
	{
		...
		try
		{
			...
			await KOTCategoryData.SaveTransaction(_kotCategory, _user.Id, platform.FormFactor, platform.Platform, platform.Latitude, platform.Longitude);
	...
	private async Task EditSelectedItem()
	{
		...
		_kotCategory = await CommonData.LoadTableDataById<KOTCategoryModel>(StoreNames.KOTCategory, selectedRecords[0].Id);
		if (_kotCategory is null)
		{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			var category = await CommonData.LoadTableDataById<KOTCategoryModel>(StoreNames.KOTCategory, id)
				?? throw new Exception("Transaction not found.");

			var platform = await PlatformInfo.GetPlatformInfo();
			if (isRecover) await KOTCategoryData.RecoverTransaction(category, _user.Id, platform.FormFactor, platform.Platform, platform.Latitude, platform.Longitude);
			else await KOTCategoryData.DeleteTransaction(category, _user.Id, platform.FormFactor, platform.Platform, platform.Latitude, platform.Longitude);
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = KOTCategoryExport.ExportMaster(_kotCategories, await CommonData.LoadCurrentDateTime(), isExcel ? ReportExportType.Excel : ReportExportType.PDF);
	...
	#region Utilities
	private async Task OnGridContextMenuItemClicked(ContextMenuClickEventArgs<KOTCategoryModel> args)
	{
		switch (args.Item.Id)
		{
