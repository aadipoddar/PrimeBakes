...
public partial class BillReport : IAsyncDisposable
{
	...
	private async Task LoadData()
	{
		...
		_customers = [.. (await customers).OrderBy(s => s.Name)];
	...
	private async Task LoadTransactionOverviews()
	{
		...
		try
		{
			...
			_allTransactionOverviews = await CommonData.LoadTableDataByDate<BillOverviewModel>(RestaurantNames.BillOverview, fromDate, toDate, useLocalDB: true);
	...
	private async Task ExportReport(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = BillReportExport.ExportReport(
	...
	private async Task ExportSelectedThermal()
	{
		...
		try
		{
			...
			var thermalBundle = await BillData.LoadThermalBundle(_sfGrid.SelectedRecords.First().Id);
